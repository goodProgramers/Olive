...
public class BrandProDAOImpl implements BrandProDAO{


	// 1. 싱글톤
	private BrandProDAOImpl() {}
	private static BrandProDAOImpl instance = new BrandProDAOImpl();
	public static BrandProDAOImpl getInstance() {
		return instance;
	}
	...
	public Map<String, List<HotProductDTO>> selectProductList(String [] brands,Connection con) throws SQLException {
		...
		ArrayList<HotProductDTO> list = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		int cnt = 0;
		...
		try {
			...
			rs = pstmt.executeQuery();
			if ( rs.next() ) {
				brProduct = new LinkedHashMap<String, List<HotProductDTO>>();
				HotProductDTO dto = null;
				dto =  new HotProductDTO();
				list = new ArrayList<HotProductDTO>();
				...
				for (int i= 0; i < brands.length; i++) {
					...
					if( brands [i].equals(rs.getString("br_name")) ) {
						...
						dto.setPrm_img( rs.getString("prm_url")); // 상품 대표 이미지
						list.add(dto);
						if( cnt < 2 ) {
							i--;
						}
